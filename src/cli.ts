...
const commands = new Map<string, CommandHandler>([
  ...
  ["auth oauth bot", authOAuthBot],
  ["auth oauth code", authOAuthCode],
  ["auth oauth login", authOAuthLogin],
  ["auth oauth url", authOAuthUrl],
  ["config path", configPath],
  ["config show", configShow],
  ["workspace current", workspaceCurrent],
  ["workspace validate", workspaceValidate],
  ["user me", userMe],
  ["whoami", userMe],
  ...
  ["issue advanced-search", issueAdvancedSearch],
  ["issue get", issueGet],
  ["issue create", issueCreate],
  ["issue update", issueUpdate],
  ["issue delete", issueDelete],
  ["issue type-schema", issueTypeSchema],
  ["issue type-list", issueTypeList],
  ["issue link list", linkList],
  ["issue link get", linkGet],
  ["issue link create", linkCreate],
  ["issue link update", linkUpdate],
  ["issue link delete", linkDelete],
  ["issue attachment list", attachmentList],
  ["issue attachment get", attachmentGet],
  ["issue attachment request-upload", attachmentRequestUpload],
  ["issue attachment complete", attachmentComplete],
  ["issue attachment upload", attachmentUpload],
  ["issue attachment update", attachmentUpdate],
  ["issue attachment delete", attachmentDelete],
  ["work-item list", issueList],
  ["work-item search", issueSearch],
  ["work-item advanced-search", issueAdvancedSearch],
  ["work-item get", issueGet],
  ["work-item create", issueCreate],
  ["work-item update", issueUpdate],
  ["work-item delete", issueDelete],
  ["work-item type-schema", issueTypeSchema],
  ["work-item type-list", issueTypeList],
  ["work-item link list", linkList],
  ["work-item link get", linkGet],
  ["work-item link create", linkCreate],
  ["work-item link update", linkUpdate],
  ["work-item link delete", linkDelete],
  ["work-item attachment list", attachmentList],
  ["work-item attachment get", attachmentGet],
  ["work-item attachment request-upload", attachmentRequestUpload],
  ["work-item attachment complete", attachmentComplete],
  ["work-item attachment upload", attachmentUpload],
  ["work-item attachment update", attachmentUpdate],
  ["work-item attachment delete", attachmentDelete],
  ["state list", resourceList("states")],
  ["state create", resourceCreate("states")],
  ["state update", resourceUpdate("states")],
  ["state delete", resourceDelete("states")],
  ["label list", resourceList("labels")],
  ["label create", resourceCreate("labels")],
  ["label update", resourceUpdate("labels")],
  ["label delete", resourceDelete("labels")],
  ["module list", resourceList("modules")],
  ["module create", resourceCreate("modules")],
  ["module update", resourceUpdate("modules")],
  ["module delete", resourceDelete("modules")],
  ["module add-item", containerAddItem("modules")],
  ["module remove-item", containerRemoveItem("modules")],
  ["module items", containerItems("modules")],
  ["cycle list", resourceList("cycles")],
  ["cycle create", resourceCreate("cycles")],
  ["cycle update", resourceUpdate("cycles")],
  ["cycle delete", resourceDelete("cycles")],
  ["cycle add-item", containerAddItem("cycles")],
  ["cycle remove-item", containerRemoveItem("cycles")],
  ["cycle items", containerItems("cycles")],
  ["page list", resourceList("pages")],
  ["page create", resourceCreate("pages")],
  ["page update", resourceUpdate("pages")],
  ["page delete", resourceDelete("pages")],
...
export const helpText = `plane-cli
...
export async function runCli(argv: string[], deps: CliDeps = {}): Promise<CliResult> {
  ...
  try {
    ...
    if (!resolved) {
      throw new ValidationAppError(`Unknown command: ${parsed.positionals.slice(0, 3).join(" ") || argv[0]}`, {
...
function resolveCommand(positionals: string[]): { handler: CommandHandler; words: number } | undefined {
...
async function authOAuthBot(context: CommandContext): Promise<CommandResult> {
  ...
  return { data: result, human: `Saved Plane OAuth app workspace ${workspace} to ${result.configPath}` };
}
...
async function authOAuthCode(context: CommandContext): Promise<CommandResult> {
  ...
  return { data: result, human: `Saved Plane OAuth user workspace ${workspace} to ${result.configPath}` };
}
...
async function issueCreate(context: CommandContext): Promise<CommandResult> {
  ...
  const data = await client.createWorkItem(
    ...
    dropUndefined({
      ...
      description: stringFlag(context, "description") ?? (await fileFlag(context, "description-file")),
...
async function issueTypeList(context: CommandContext): Promise<CommandResult> {
  const { client, project, workspace } = await clientAndProject(context);
  return { data: await client.listWorkItemTypes(String(project.id), queryFlags(context)), workspace };
}
...
async function linkList(context: CommandContext): Promise<CommandResult> {
  ...
  return { data: await client.listLinks(String(project.id), issueId, queryFlags(context)), workspace };
}
...
async function linkGet(context: CommandContext): Promise<CommandResult> {
  ...
  return {
    data: await client.getLink(String(project.id), issueId, requiredArg(context, 0, "link"), queryFlags(context)),
    workspace,
  };
}
...
async function attachmentList(context: CommandContext): Promise<CommandResult> {
  const { client, issueId, project, workspace } = await clientProjectIssue(context, 0);
  return { data: await client.listAttachments(String(project.id), issueId, queryFlags(context)), workspace };
}

async function attachmentGet(context: CommandContext): Promise<CommandResult> {
  const { client, issueId, project, workspace } = await clientProjectIssue(context, 1);
  return {
    data: await client.getAttachment(String(project.id), issueId, requiredArg(context, 0, "attachment")),
    workspace,
  };
}
...
async function attachmentRequestUpload(context: CommandContext): Promise<CommandResult> {
  const { client, issueId, project, workspace } = await clientProjectIssue(context, 0);
  return {
    data: await client.requestAttachmentUpload(String(project.id), issueId, attachmentUploadPayload(context)),
    workspace,
  };
}
...
async function attachmentComplete(context: CommandContext): Promise<CommandResult> {
  const { client, issueId, project, workspace } = await clientProjectIssue(context, 1);
  return {
    data: await client.completeAttachmentUpload(String(project.id), issueId, requiredArg(context, 0, "attachment")),
    workspace,
  };
}
...
async function attachmentDelete(context: CommandContext): Promise<CommandResult> {
  ...
  const { client, issueId, project, workspace } = await clientProjectIssue(context, 1);
  return {
    data: await client.deleteAttachment(String(project.id), issueId, requiredArg(context, 0, "attachment")),
    workspace,
  };
}
...
function resourceList(resource: ProjectResource): CommandHandler {
  return async (context) => {
    const { client, project, workspace } = await clientAndProject(context);
    return { data: await client.listResource(String(project.id), resource, queryFlags(context)), workspace };
  };
}
...
function resourceCreate(resource: ProjectResource): CommandHandler {
  return async (context) => {
    const { client, project, workspace } = await clientAndProject(context);
    const data = await client.createResource(String(project.id), resource, resourcePayload(context));
    return { data, workspace };
  };
}
...
function resourceDelete(resource: ProjectResource): CommandHandler {
  return async (context) => {
    ...
    const { client, project, workspace } = await clientAndProject(context);
    return {
      data: await client.deleteResource(String(project.id), resource, requiredArg(context, 0, "id")),
      workspace,
    };
  };
}
...
function containerAddItem(resource: "cycles" | "modules"): CommandHandler {
  return async (context) => {
    ...
    return {
      data: await client.addItemToContainer(String(project.id), resource, requiredArg(context, 0, "container"), [issueId]),
      workspace,
    };
  };
}
...
function containerRemoveItem(resource: "cycles" | "modules"): CommandHandler {
  return async (context) => {
    ...
    const issueId = await resolveIssueId(client, project, requiredArg(context, 1, "issue"));
    return {
      data: await client.removeItemFromContainer(String(project.id), resource, requiredArg(context, 0, "container"), issueId),
      workspace,
    };
  };
}
...
function containerItems(resource: "cycles" | "modules"): CommandHandler {
  return async (context) => {
    const { client, project, workspace } = await clientAndProject(context);
    return {
      data: await client.listContainerItems(String(project.id), resource, requiredArg(context, 0, "container")),
      workspace,
    };
  };
}
...
async function commentCreate(context: CommandContext): Promise<CommandResult> {
  const { client, project, workspace } = await clientAndProject(context);
  const issueId = await resolveIssueId(client, project, requiredArg(context, 0, "issue"));
  return { data: await client.createComment(String(project.id), issueId, requiredFlag(context, "body")), workspace };
}
...
async function commentUpdate(context: CommandContext): Promise<CommandResult> {
  ...
  const issueId = await resolveIssueId(client, project, requiredArg(context, 1, "issue"));
  return {
    data: await client.updateComment(String(project.id), issueId, requiredArg(context, 0, "comment"), requiredFlag(context, "body")),
    workspace,
  };
}
...
async function commentDelete(context: CommandContext): Promise<CommandResult> {
  ...
  const { client, project, workspace } = await clientAndProject(context);
  const issueId = await resolveIssueId(client, project, requiredArg(context, 1, "issue"));
  return { data: await client.deleteComment(String(project.id), issueId, requiredArg(context, 0, "comment")), workspace };
}

async function clientFor(context: CommandContext): Promise<{ client: PlaneClient; workspace: WorkspaceConfig }> {
  const resolution = await resolveCurrentWorkspace(context);
  return {
...
async function clientAndProject(
  ...
  const { client, workspace } = await clientFor(context);
  return { client, project: await getProjectForLookup(client, stringFlag(context, "project") ?? (await repoProject(context))), workspace };
}

async function getProjectForLookup(client: PlaneClient, lookup: string | undefined): Promise<JsonObject> {
  if (!lookup) {
...
): Promise<{ client: PlaneClient; issueId: string; project: JsonObject; workspace: WorkspaceConfig }> {
  const { client, project, workspace } = await clientAndProject(context);
  const issueId = await resolveIssueId(client, project, requiredArg(context, issueArgIndex, "issue"));
...
async function resolveIssueId(client: PlaneClient, project: JsonObject, issue: string): Promise<string> {
...
async function discoverWorkspaceSlugFromAppInstallation(options: {
  ...
  if (response.status === 401 || response.status === 403) {
    throw new AppError("PLANE_AUTH_FAILED", "Plane rejected the OAuth token while discovering app installation.", exitCodes.auth, {
      status: response.status,
    });
  }
  if (response.status === 404) {
    throw new AppError("NOT_FOUND", "Plane app installation endpoint was not found.", exitCodes.notFound, {
      status: response.status,
    });
  }
  if (response.status < 200 || response.status >= 300) {
    throw new AppError("API_ERROR", `Plane app installation lookup failed with status ${response.status}`, exitCodes.api, {
      body,
      status: response.status,
    });
  }
  ...
  const match = matchWorkspace(installations, options.workspace) ?? (installations.length === 1 ? installations[0] : undefined);
  const slug = match ? workspaceSlugFromObject(match) : undefined;
  if (!slug) {
    throw new ValidationAppError("Could not discover the Plane workspace slug from the app installation.", {
...
async function discoverWorkspaceSlug(options: {
  ...
  const fetcher = options.fetch ?? fetch;
  const response = await fetcher(new URL("/api/workspaces/", `${options.baseUrl.replace(/\/+$/, "")}/`).toString(), {
  ...
  const body = text ? safeJsonObject(text) : {};
  if (response.status === 401 || response.status === 403) {
    throw new AppError("PLANE_AUTH_FAILED", "Plane rejected the credentials while discovering workspaces.", exitCodes.auth, {
      status: response.status,
    });
  }
  if (response.status === 404) {
    throw new AppError("NOT_FOUND", "Plane workspace discovery endpoint was not found.", exitCodes.notFound, {
      status: response.status,
    });
  }
  if (response.status < 200 || response.status >= 300) {
    throw new AppError("API_ERROR", `Plane workspace discovery failed with status ${response.status}`, exitCodes.api, {
      body,
      status: response.status,
    });
  }
  ...
  const match = matchWorkspace(workspaces, options.workspace) ?? (workspaces.length === 1 ? workspaces[0] : undefined);
  const slug = match ? workspaceSlugFromObject(match) : undefined;
  if (!slug) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const server = createServer((request, response) => {
      try {
        ...
        if (state !== options.state) throw new ValidationAppError("Plane OAuth callback state did not match.");
        response.writeHead(200, { "Content-Type": "text/html" });
        response.end("<html><body><h1>Plane CLI login complete</h1><p>You can close this window.</p></body></html>");
